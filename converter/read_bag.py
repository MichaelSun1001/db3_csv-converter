...
def read_from_topic(bag_file, topic_name, print_out=False):
    ...
    timestamps, messages_cdr = get_all_msgs_from_topic(
        cursor, topic_name, print_out=False)
...
def read_from_all_topics(bag_file, print_out=False):
    ...
    for i in range(len(topic_names)):
        timestamps, messages = read_from_topic(
            bag_file, topic_names[i], print_out)
...
def read_write_from_all_topics(bag_file, print_out=False):
    ...
    """
    # 使用绝对路径
    # bag_file = Path(bag_file).resolve()
    # Connect to the database
    connection, cursor = connect(bag_file)
    ...
    for i in range(len(topic_names)):
        ...
        save_csv_file.save_csv_file(
            [timestamps, messages], path + "/" + topic_name+".csv")
